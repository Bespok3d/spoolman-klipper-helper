# spoolman-klipper-helper: instructions for AI assistants
...
This is a solo plugin repo: it ships one plugin, `spoolman`, zero-config Spoolman integration that
keeps the active spool always tracked. It publishes a single index atom into
`Bespok3d/main-index/atoms/` and no list of its own.

The plugin carries Python: Klipper/Moonraker extras under `files/klipper/klippy/extras/spoolman` (plus
`spoolman_helper.py`), not just config, patches, and assets.
